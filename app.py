...
# EV(期待値)判定機能
#
# v1.5.3で計算しているhead_score(1着適性スコア)を確率に変換し、Harville法で
# 3連単の確率を推定、リアルタイムの3連単オッズと突き合わせて
# 「モデル確率 × オッズ - 1」がプラスに大きい買い目だけを候補として示す。
#
...
def parse_odds3t(src):
    ...
    app.py本体のextract_rows/extract_cellsをそのまま使い、
    新規の依存(BeautifulSoup等)を増やさない。"""
    odds = {}
...
def compute_ev_table(probs, odds, ev_threshold_pct, bet_amount=100):
    ...
    for combo, p in probs.items():
        ...
        rows.append({
            "combo": combo,
            "model_prob_pct": round(p * 100.0, 2),
            "odds": o,
    ...
    if len(odds) < MIN_ODDS_COVERAGE:
        # オッズ取得が不完全な状態でのEVは信用できないため、候補は出さない
        candidates = []
    else:
...
def analyze(jcd, rno, hd, ev_threshold=DEFAULT_EV_THRESHOLD):
    key = f"{hd}:{jcd}:{rno}:{ev_threshold}"
    now = time.time()
...
def api_analyze():
    try:
        ...
        data = analyze(jcd, rno, hd, ev_threshold=ev_threshold)
